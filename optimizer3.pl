...
sub evaluate {
    ...
    my $cmd = "./src/pmars -r 1000 -s 8000 -c 80000 -p 8000 -l 100 -d 100 temp_opt.red doc/examples/validate.red";
    my $out = `$cmd`;
    my @lines = split(/\n/, $out);
    if (@lines) {
...
if ($best_s1 != 1761 || $best_s2 != 2407) {
    print "Updating warrior.red with steps: ($best_s1, $best_s2)\n";
    open(my $fh, '>', 'warrior.red') or die $!;
    print $fh make_warrior($best_s1, $best_s2);
    close($fh);
} else {
    print "Baseline remains the best.\n";
}
